// Hash router SPA. Namespace #/p1-m4-p{1..12}
// Triagem Neonatal — 6 triagens
...
(function () {
  ...
  const HASH_PREFIX = '#/p1-m4-p';

  function parseHash() {
    const h = location.hash || HASH_PREFIX + '1';
    const m = /^#\/p1-m4-p(\d+)$/.exec(h);
    if (!m) return { page: 1 };
  ...
  function updateTitle(pageNum) {
    ...
    const base = 'Triagem Neonatal — Neonatologia (Parte 1, M4)';
  ...
  function bindTheme() {
    ...
    if (!btn) return;
    const saved = localStorage.getItem('neonato-p1-m4.theme');
    if (saved === 'light') {
    ...
    btn.addEventListener('click', () => {
      ...
      if (cur === 'light') {
        ...
        localStorage.setItem('neonato-p1-m4.theme', 'dark');
      } else {
        document.documentElement.setAttribute('data-theme', 'light');
        btn.setAttribute('aria-pressed', 'true');
        localStorage.setItem('neonato-p1-m4.theme', 'light');
      }
    });
  }
  ...
  function bindBauerRevisable() {
    // Toggle expand/collapse no laudo dual
