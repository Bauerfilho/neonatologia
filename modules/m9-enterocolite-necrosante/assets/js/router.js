// Hash router SPA. Namespace #/p2-m9-p{1..8}
// Enterocolite Necrosante — M9 (Parte 2, Bônus 1)
...
(function () {
  ...
  const PAGES_TOTAL = 8;
  const HASH_PREFIX = '#/p2-m9-p';
  const STORAGE_PREFIX = 'neonato-p2-m9';

  function parseHash() {
    const h = location.hash || HASH_PREFIX + '1';
    const m = /^#\/p2-m9-p(\d+)$/.exec(h);
    if (!m) return { page: 1 };
  ...
  function updateTitle(pageNum) {
    ...
    const base = 'Enterocolite Necrosante — Neonatologia (Parte 2, Bônus 1)';
  ...
  function onHashChange() {
    ...
    showPage(page);
    document.dispatchEvent(new CustomEvent('m9:pagechange', { detail: { page } }));
  }
