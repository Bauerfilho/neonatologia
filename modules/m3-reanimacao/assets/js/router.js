// Hash router SPA. Namespace #/p1-m3-p{1..12}
// Mantém histórico do navegador, fecha drawer, atualiza progresso e ARIA.

(function () {
  'use strict';

  const PAGES_TOTAL = 12;
  const HASH_PREFIX = '#/p1-m3-p';
  const VALID_PAINEL = ['painel-1', 'painel-2', 'painel-3'];

  function parseHash() {
    const h = location.hash || HASH_PREFIX + '1';
    const m = /^#\/p1-m3-p(\d+)(?:#(painel-\d+))?$/.exec(h);
    if (!m) return { page: 1, painel: null };
    let page = parseInt(m[1], 10);
    if (isNaN(page) || page < 1) page = 1;
    if (page > PAGES_TOTAL) page = PAGES_TOTAL;
    const painel = m[2] && VALID_PAINEL.includes(m[2]) ? m[2] : null;
    return { page, painel };
  }

  function showPage(pageNum, painel) {
    ...
    if (!found) {
      // Página não renderizada — exibe a 1
  ...
  function updateTitle(pageNum) {
    ...
    const base = 'Reanimação Neonatal — Neonatologia (Parte 1, M3)';
  ...
  function bindTheme() {
    ...
    if (!btn) return;
    const saved = localStorage.getItem('neonato-p1-m3.theme');
    if (saved === 'light') {
    ...
    btn.addEventListener('click', () => {
      ...
      if (cur === 'light') {
        ...
        localStorage.setItem('neonato-p1-m3.theme', 'dark');
      } else {
        document.documentElement.setAttribute('data-theme', 'light');
        btn.setAttribute('aria-pressed', 'true');
        localStorage.setItem('neonato-p1-m3.theme', 'light');
      }
    });
  }

  function bindPainelNav() {
    document.querySelectorAll('.painel-nav__btn').forEach((b) => {
      b.addEventListener('click', (e) => {
        const targetId = b.dataset.painel;
        if (!targetId) return;
        e.preventDefault();
        document.querySelectorAll('.painel-nav__btn').forEach((x) => x.setAttribute('aria-current', 'false'));
        b.setAttribute('aria-current', 'true');
        const el = document.getElementById(targetId);
        if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
        history.replaceState(null, '', HASH_PREFIX + '12#' + targetId);
      });
    });
  }

  function bindChecklist() {
    document.querySelectorAll('.final-checklist li').forEach((li) => {
      const key = li.dataset.checkKey;
      if (!key) return;
      const fullKey = 'neonato-p1-m3.checklist.' + key;
      if (localStorage.getItem(fullKey) === 'true') li.dataset.checked = 'true';
      li.addEventListener('click', () => {
        const isOn = li.dataset.checked === 'true';
        li.dataset.checked = isOn ? 'false' : 'true';
        localStorage.setItem(fullKey, String(!isOn));
      });
