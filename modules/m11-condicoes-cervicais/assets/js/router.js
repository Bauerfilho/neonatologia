// Hash router SPA. Namespace #/p2-m11-p{1..7}
// M11 — Condições cervicais cirúrgicas em pediatria (Bônus 3)
...
(function () {
  ...
  const PAGES_TOTAL = 7;
  const HASH_PREFIX = '#/p2-m11-p';
  const STORAGE_PREFIX = 'neonato-p2-m11';

  function parseHash() {
    const h = location.hash || HASH_PREFIX + '1';
    const m = /^#\/p2-m11-p(\d+)$/.exec(h);
    if (!m) return { page: 1 };
  ...
  function updateTitle(pageNum) {
    ...
    const base = 'Condições cervicais cirúrgicas — Neonatologia (Parte 2, Bônus 3 · M11)';
  ...
  function onHashChange() {
    ...
    showPage(page);
    document.dispatchEvent(new CustomEvent('m11:pagechange', { detail: { page } }));
  }
