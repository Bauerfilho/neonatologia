// Hash router SPA. Namespace #/p2-m12-p{1..17}
// M12 — Defeitos neonatais / Cirurgia pediátrica (Bônus 4)
// Mantém histórico do navegador, fecha drawer, atualiza progresso e ARIA.

(function () {
  'use strict';

  const PAGES_TOTAL = 17;
  const HASH_PREFIX = '#/p2-m12-p';
  const STORAGE_PREFIX = 'neonato-p2-m12';

  function parseHash() {
    const h = location.hash || HASH_PREFIX + '1';
    const m = /^#\/p2-m12-p(\d+)$/.exec(h);
    if (!m) return { page: 1 };
  ...
  function updateTitle(pageNum) {
    ...
    const base = 'Defeitos neonatais / Cirurgia pediátrica — Neonatologia (Parte 2, Bônus 4 · M12)';
  ...
  function onHashChange() {
    ...
    showPage(page);
    document.dispatchEvent(new CustomEvent('m12:pagechange', { detail: { page } }));
  }
  ...
  // Cascade alcalose interativa (P12) — clique em cada passo expande detalhe
  function bindCascadeSteps() {
  ...
  // VACTERL letras com hover/foco (P13) — tooltip expande explicação
  function bindVACTERLLetters() {
    document.querySelectorAll('.vacterl-letter').forEach((el) => {
      el.addEventListener('click', () => {
        const expanded = el.dataset.expanded === 'true';
        document.querySelectorAll('.vacterl-letter').forEach((s) => { s.dataset.expanded = 'false'; });
